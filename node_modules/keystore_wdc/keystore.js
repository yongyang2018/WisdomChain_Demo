...
class KeyStore {
    ...
    CreateMultipleToDeployforRuleOther(sign,rawTransaction,transaction_pubkey,fromPubkeyStr,prikeyStr){
        return new Transaction().CreateMultipleToDeployforRuleOther(sign,rawTransaction,transaction_pubkey,fromPubkeyStr,prikeyStr);
    }
    //多签部署组装事务（用于广播）
    CreateMultipleForRuleSplice(signTransaction,fromPubkeyStr_other,sign_other,prikeyStr){
        return new Transaction().CreateMultipleForRuleSplice(signTransaction,fromPubkeyStr_other,sign_other,prikeyStr);
    }
    //多签转账（签发者构建）
    CreateMultisignatureForTransferFirst(fromPubkeyStr,toPubkeyHashStr,nonceNum,prikeyStr,payload_origin,payload_dest,payload_to,payload_value,payload_pubkeyHashasBytesArray){
        return new Transaction().CreateMultisignatureForTransferFirst(fromPubkeyStr,toPubkeyHashStr,nonceNum,prikeyStr,payload_origin,payload_dest,payload_to,payload_value,payload_pubkeyHashasBytesArray);
    }
    //多签转账(其他人签名)
    CreateMultisignatureToDeployforRuleOther(sign,rawTransaction,transaction_pubkey,fromPubkeyStr,prikeyStr){
        return new Transaction().CreateMultisignatureToDeployforRuleOther(sign,rawTransaction,transaction_pubkey,fromPubkeyStr,prikeyStr);
    }
    //多签转账组装事务（用于广播）
    CreateMultisignatureForTransferSplice(signTransaction,fromPubkeyStr_other,sign_other,prikeyStr){
        return new Transaction().CreateMultisignatureForTransferSplice(signTransaction,fromPubkeyStr_other,sign_other,prikeyStr);
    }
    //定额条件比例支付部署
    CreateRateheightlockruleForDeploy(fromPubkeyStr, prikeyStr,nonce,assetHash,onetimedepositmultiple,withdrawperiodheight,withdrawrate,dest){
        return new Transaction().CreateRateheightlockruleForDeploy(fromPubkeyStr,prikeyStr,nonce,assetHash,onetimedepositmultiple,withdrawperiodheight,withdrawrate,dest);
    }
    //定额条件比例支付转入
    CreateRateheightlockDepositRuleForDeploy(fromPubkey,prikeyStr,txHash,nonceNum,value){
        return new Transaction().CreateRateheightlockDepositRuleForDeploy(fromPubkey,prikeyStr,txHash,nonceNum,value);
    }
    //定额条件比例支付转出
    CreateRateheightlockWithdrawRuleForDeploy(fromPubkey,txHash,prikeyStr,nonceNum,deposithash,to){
        return new Transaction().CreateRateheightlockWithdrawRuleForDeploy(fromPubkey,txHash,prikeyStr,nonceNum,deposithash,to)
    }
}
