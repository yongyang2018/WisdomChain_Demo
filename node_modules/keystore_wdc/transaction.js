...
class Transaction
{
    ...
    CreateMultipleForRuleFirst(fromPubkeyStr,nonceNum,prikeyStr,payload_asset160hash,payload_m,payload_n,payload_pubkeyHashasBytesArray){
        try {
            ...
            let sigHex = this.Bytes2Str(sigall).substring(0,128);
            //公钥、签名放入payload中
            let pubkeys = new Array();
            let signs = new Array();
            pubkeys.push(Buffer.from(fromPubkeyStr,'hex'));
            signs.push(Buffer.from(sigHex,'hex'));
            const MultisignRule_first = [Buffer.from(payload_asset160hash,'hex'),payload_m*1,payload_n*1,pubkeys,signs,payload_pubkeyHashasBytesArray];
            const MultisignRule_firstEncoded = RLP.encode(MultisignRule_first);
            let payloadLen_first = uint32.toHex(MultisignRule_firstEncoded.length+1);
            let payload_first = "01"+MultisignRule_firstEncoded.toString('hex');
            let allPayload_first = payloadLen_first + payload_first;
            let signTransaction_first = version+this.Bytes2Str(Buffer.alloc(32))+type+nonce+fromPubkeyHash+gasPrice+Amount+sigHex+toPubkeyHash+allPayload_first;
            return {
                ...
                'signTransaction':signTransaction_first
            }
        } catch (error) {
            console.log(error);
            return 5000;
    ...
    //部署多签(其他人签名)
    CreateMultipleToDeployforRuleOther(sign,rawTransaction,transaction_pubkey,fromPubkeyStr,prikeyStr){
        try {
            //校验签名
            let isSign = nacl.sign.detached.verify(Buffer.from(rawTransaction,'hex'),Buffer.from(sign,'hex'),Buffer.from(transaction_pubkey,'hex'));
            if(!isSign){
                return 5100;
            }
            let secretKey = Buffer.from(prikeyStr+fromPubkeyStr, 'hex');
    ...
    //多签部署组装事务（用于广播）
    CreateMultipleForRuleSplice(signTransaction,fromPubkeyStr_other,sign_other,prikeyStr){
        try{
            let transaction = this.AnalysisTransaction(signTransaction);
            prikeyStr = this.DismantlingPrivateByLength(prikeyStr);
            //版本号
            ...
            let toPubkeyHash=this.Bytes2Str(Buffer.alloc(20));
            //from、sign数组拼接
            let fromArrays = transaction.payload_decoded[3];
            let signs = transaction.payload_decoded[4];
            fromArrays.push(Buffer.from(fromPubkeyStr_other,'hex'));
            signs.push(Buffer.from(sign_other,'hex'));
            //payload
            const MultisignRule = [transaction.payload_decoded[0],transaction.payload_decoded[1],transaction.payload_decoded[2],fromArrays,signs,transaction.payload_decoded[5]];
            const MultisignRuleEncoded = RLP.encode(MultisignRule);
            let payloadLen = uint32.toHex(MultisignRuleEncoded.length+1);
            let payload = "01"+MultisignRuleEncoded.toString('hex')
            let allPayload = payloadLen + payload;
            ...
            let sigall = nacl.sign(RawTransaction,secretKey);
            let sigHexall = this.Bytes2Str(sigall).substring(0,128);
            let _tra = version+type+nonce+fromPubkey+gasPrice+Amount+sigHexall+toPubkeyHash+allPayload;
            let tra = Buffer.from(_tra,'hex');
            let transha = keccak256(tra);
            let signRawBasicTransaction = version+transha+type+nonce+fromPubkey+gasPrice+Amount+sigHexall+toPubkeyHash+allPayload;
            return {
                'txHash': transha,
                'signTransaction': version+type+nonce+fromPubkey+gasPrice+Amount+signull+toPubkeyHash+allPayload,
                'allsignTransaction':signRawBasicTransaction
            }
        } catch (error) {
            console.log(error);
            return 5000;
    ...
    //多签转账（签发者构建）
    CreateMultisignatureForTransferFirst(fromPubkey,txHash,nonceNum,prikeyStr,payload_origin,payload_dest,payload_to,payload_value,payload_pubkeyHashasBytesArray){
        try {
            prikeyStr = this.DismantlingPrivateByLength(prikeyStr);
            //版本号
            let version="01";
            ...
            pubkeys.push(Buffer.from(fromPubkeyStr,'hex'));
            signs.push(Buffer.from(sigHex,'hex'));
            const MultisignRule_first = [payload_origin*1,payload_dest*1,pubkeys,signs,Buffer.from(payload_to,'hex'),mul*1,payload_pubkeyHashasBytesArray];
            const MultisignRule_firstEncoded = RLP.encode(MultisignRule_first);
            let payloadLen_first = uint32.toHex(MultisignRule_firstEncoded.length+1);
            let payload_first = "03"+MultisignRule_firstEncoded.toString('hex')
            let allPayload_first = payloadLen_first + payload_first;
            let RawTransaction_first = version+type+nonce+fromPubkeyStr+gasPrice+Amount+signull+toPubkeyHash+allPayload_first;
            return {
                'fromPubkeyStr': fromPubkeyStr,
                'sign': sigHex,
                'rawTransaction':version+type+nonce+fromPubkeyStr+gasPrice+Amount+signull+toPubkeyHash+allPayload,
                'signTransaction':RawTransaction_first
            }
        } catch (error) {
            console.log(error);
            return 5000;
    ...
    //多签转账(其他人签名)
    CreateMultisignatureToDeployforRuleOther(sign,rawTransaction,transaction_pubkey,fromPubkeyStr,prikeyStr){
        try {
            //校验签名
            let isSign = nacl.sign.detached.verify(Buffer.from(rawTransaction,'hex'),Buffer.from(sign,'hex'),Buffer.from(transaction_pubkey,'hex'));
            if(!isSign){
    ...
    //多签转账组装事务（用于广播）
    CreateMultisignatureForTransferSplice(signTransaction,fromPubkeyStr_other,sign_other,prikeyStr){
        try {
            let transaction = this.AnalysisTransaction(signTransaction);
            prikeyStr = this.DismantlingPrivateByLength(prikeyStr);
            //版本号
            ...
            //from、sign数组拼接
            let fromArrays = transaction.payload_decoded[2];
            let signs = transaction.payload_decoded[3];
            fromArrays.push(Buffer.from(fromPubkeyStr_other,'hex'));
            signs.push(Buffer.from(sign_other,'hex'));
            //payload
            const MultisignRule = [transaction.payload_decoded[0],transaction.payload_decoded[1],fromArrays,signs,transaction.payload_decoded[4],transaction.payload_decoded[5],transaction.payload_decoded[6]];
            const MultisignRuleEncoded = RLP.encode(MultisignRule);
            let payloadLen = uint32.toHex(MultisignRuleEncoded.length+1);
    ...
    //部署定额条件比例支付
    CreateRateheightlockruleForDeploy(fromPubkey,prikeyStr,nonceNum,assetHash,onetimedepositmultiple,withdrawperiodheight,withdrawrate,dest){
        try {
            prikeyStr = this.DismantlingPrivateByLength(prikeyStr);
            //版本号
            let version="01";
            ...
            let toPubkeyHash = this.Bytes2Str(Buffer.alloc(20));
            //payload
            const Rateheightlock = [Buffer.from(assetHash,'hex'),onetimedepositmultiple*1,withdrawperiodheight*1,withdrawrate.toString(),Buffer.from(dest,'hex'),new Array()];
            const RateheightlockEncoded = RLP.encode(Rateheightlock);
            let payloadLen = uint32.toHex(RateheightlockEncoded.length+1);
            let payload = "04"+RateheightlockEncoded.toString('hex');
            let allPayload = payloadLen + payload;
            let RawTransaction=Buffer.from(version+type+nonce+fromPubkeyStr+gasPrice+Amount+signull+toPubkeyHash+allPayload, 'hex');
            //签名数据
            let secretKey = Buffer.from(prikeyStr+fromPubkeyStr, 'hex');
    ...
    //定额条件比例支付转入事务
    CreateRateheightlockDepositRuleForDeploy(fromPubkey,prikeyStr,txHash,nonceNum,value){
        try {
            prikeyStr = this.DismantlingPrivateByLength(prikeyStr);
            //版本号
            let version="01";
            ...
            //接收者公钥哈希 0填充
            let toPubkeyHash = txHash;
            let mul = this.mul(value);
            if(mul < 0){
                return 5000;
            }
            //payload
            const RateheightlockDeposit = [mul*1];
            const RateheightlockDepositEncoded = RLP.encode(RateheightlockDeposit);
            let payloadLen = uint32.toHex(RateheightlockDepositEncoded.length+1);
            let payload = "08"+RateheightlockDepositEncoded.toString('hex');
            let allPayload = payloadLen + payload;
            let RawTransaction=Buffer.from(version+type+nonce+fromPubkeyStr+gasPrice+Amount+signull+toPubkeyHash+allPayload, 'hex');
            //签名数据
            let secretKey = Buffer.from(prikeyStr+fromPubkeyStr, 'hex');
    ...
    //定额条件比例支付转出事务
    CreateRateheightlockWithdrawRuleForDeploy(fromPubkey,txHash,prikeyStr,nonceNum,deposithash,to){
        try {
            prikeyStr = this.DismantlingPrivateByLength(prikeyStr);
            //版本号
            let version="01";
            ...
            let toPubkeyHash = txHash;
            //payload
            const RateheightlockWithdraw = [Buffer.from(deposithash,'hex'),Buffer.from(to,'hex')];
            const RateheightlockWithdrawEncoded = RLP.encode(RateheightlockWithdraw);
            let payloadLen = uint32.toHex(RateheightlockWithdrawEncoded.length+1);
            let payload = "09"+RateheightlockWithdrawEncoded.toString('hex');
            let allPayload = payloadLen + payload;
            let RawTransaction=Buffer.from(version+type+nonce+fromPubkeyStr+gasPrice+Amount+signull+toPubkeyHash+allPayload, 'hex');
            //签名数据
            let secretKey = Buffer.from(prikeyStr+fromPubkeyStr, 'hex');
    ...
    AnalysisTransaction(transaction){
        ...
        if(payloadlen*1 != 0){
            //payload 类型
            let payload_type = transaction.substr(356,2);
            let len = parseInt(payloadlen,16).toString(10);
            let payload_RLP = transaction.substr(358,transaction.length-358);
